#!/bin/bash
# waves-gate.sh — PreToolUse hook for Waves 3.0 (plugin-native)
...
#
# Plugin-native: markers live session-scoped under
#   $CLAUDE_PLUGIN_DATA/markers/$SESSION_ID/  (session_id from stdin JSON).
#
# Input: stdin JSON with {session_id, tool_name, tool_input} from PreToolUse event
  ...
  if [ "$TOOL_NAME" = "Bash" ]; then
    ...
    case "$FIRST_CHECK" in
      ...
      rm)
        ...
        if echo "$COMMAND_CHECK" | grep -qE 'markers/|waves-' 2>/dev/null; then
          echo '{}'
          exit 0
        fi
        ;;
